...
impl Query {
    ...
    pub fn limit<T: Into<Option<u16>>>(mut self, limit: T) -> Self {
...
impl Client {
    ...
    async fn tag_search_page(
        &self,
        query: Option<Query>,
    ) -> Option<(Vec<Rs621Result<Tag>>, Option<Query>)> {
        ...
        // TODO: This allocates an unnecessary `Vec`.
        let tag_results = tags.into_iter().map(Ok).collect();

        Some((tag_results, Some(query)))
    }
}
